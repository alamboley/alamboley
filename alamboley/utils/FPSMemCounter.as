package alamboley.utils {
	...
	/**
	 * @author Aymeric based on the works of @author shanem http://lostinactionscript.com/
	 */
	public class FPSMemCounter {
		...
		public static function init(swf:Stage, context:InteractiveObject, visibleAtStart:Boolean = false):void {
			if (inited) return;
			...
			cm.hideBuiltInItems();
			ci = new ContextMenuItem("Show Memory Usage", true);
		...
		private static function onSelect(e:ContextMenuEvent = null):void {
			if (!displayed) {
				show();
			} else {
				hide();
